...
public class MainWS {
	...
	public GoralRoundJson createNewRound(@Context HttpServletRequest request,
			GoralRoundJson data ){
		//Get the current session
		HttpSession session = request.getSession();
	...
	public List getList(@Context HttpServletRequest request,
			@PathParam  ("id" )Integer id){
		
		HttpSession session = request.getSession();
		
		//get the right generator of this session
		GoralRoundGenerator2 g = (GoralRoundGenerator2)session.getAttribute("GoralRoundGenerator");
		
		
		//get the right goralRound of this current round
		GoralRound goralRound = g.getInstances().get(id);
		
		System.out.println("get this list: \n" + goralRound.getUserList().toString());
	...
	/**
	 * Web Service function that add new range to the list 
	 * @param request
	 * @param data
	 * @return
	 */
	...
	public GoralRoundJson addRange3(@Context HttpServletRequest request,
		...
		//use the id that passed from the client side
		Integer id = data.getId();
		
		//Get the right goralRound by the id
		GoralRound goralRound = g.getInstances().get(id);
		
		//Adding range to the list
		goralRound.addNumbers(data.getFrom(), data.getTo());
		System.out.println("Added new range to the list from " + data.getFrom() + " to " + data.getTo() +" with id " 
				+ data.getId() );
		GoralRoundJson data2 = new GoralRoundJson(data.getFrom(), data.getTo(), data.getId(),null);
		return data2 ;
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public void addSingles(@Context HttpServletRequest request,
			GoralRoundJson data ){
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public void removeSingles(@Context HttpServletRequest request,
			GoralRoundJson data ){
		
		//get the list for removing.
		Integer[] someNumbers = data.getSomeNumbers();
		//Get the current session
		HttpSession session = request.getSession();
